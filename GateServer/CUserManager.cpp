...
bool CUserManager::Init()
{
	...
	m_uiPreSecLogin = GATE_LOGIC_INS->m_oConstConfig.m_uiCheckMax;
	return true;
}
...
void CUserManager::OnTimer(uint32 uiDwTm)
{
	...
	if (m_stOnLineUp.On(uiDwTm) && m_bLoadFinish)
	{
		...
		Log_Custom("userlist", "ingame:%u, waitcheckPwd:%u, cheking:%u, checkingpwd:%u, cdache:%d, maxmsg:%u",
			...
			m_mapCheckingPwd.size(), m_pUserPool->GetPoolSize(), GATE_LOGIC_INS->GetMaxSend());
	}
}
...
void CUserManager::OnTimerCheckPwd()
{
	...
	uint32 m_uiWaitCheck = GATE_LOGIC_INS->m_oConstConfig.m_uiWaitCheck;
	...
	while (itWaitCheck != m_lstWaitCheckPwd.end())
	{
		...
		{
			itWaitCheck++;
			GATE_LOGIC_INS->m_pClientLIF->OnDisconnect(pPlayer);
			Log_Error("login wait so long, user name=%s, accountid=%lld", pPlayer->m_strAccName.c_str(), pPlayer->m_llUid);
			continue;
		}

		//已经达到游戏服上限,登录等待上线
		if (IsFull() || uiCount >= GATE_LOGIC_INS->m_oConstConfig.m_uiCheckMax)
		{
			itWaitCheck++;
			continue;
		}
	...
	//百分之90负载连续36000次报警一次
	if ((m_mapInGame.size() + m_mapChecking.size()) >= GATE_LOGIC_INS->m_oConstConfig.m_uiLimit*0.9f && (iTimes % 36000 == 0))
	{
		char szLog[4096];
		sprintf_safe(szLog, "ingame:%u, waitcheck:%u,  checking:%u, checkingpwd:%u, limit:%u, checkmax:%u, times:%d",
			...
			GATE_LOGIC_INS->m_oConstConfig.m_uiLimit, GATE_LOGIC_INS->m_oConstConfig.m_uiCheckMax, iTimes);
...
{
	time_t llCurTime = GetCurrTime();
	uint32 uiTimeOut = GATE_LOGIC_INS->m_oConstConfig.m_uiTimeout;
	...
	msg_id_t usCmd, std::map<uint16, std::vector<uint16>>& stThreadToConnMap)
{
	...
	for (auto& value : m_mapThreadToConn)
	{
		...
		my_multicast_conn_msg(GATE_LOGIC_INS->m_pLogic->Dispatcher(), 
...
bool CUserManager::SendToPlayer(int64 llUid, const tagMsgHead* pNetMsgHead)
{
	...
	CUser* pPlayer = GATE_LOGIC_INS->m_oUserMgr.GetInGamePlayer(llUid);
	if (pPlayer == nullptr)
	{
...
void CUserManager::OnTimerLineUp()
{
	...
	if (IsFull() || uiSum > GATE_LOGIC_INS->m_oConstConfig.m_uiCheckMax)
	{
		uint32 uiCurIndex = 0;
		for (auto& player : m_lstWaitCheckPwd)
		{
			...
			player->m_llNextLineUpPushTime = llCurTime + GATE_LOGIC_INS->m_oConstConfig.m_uiNotifyInterval;
		}
	}
}
...
{
	return (m_mapInGame.size() + m_mapChecking.size() + m_mapCheckingPwd.size()) >= GATE_LOGIC_INS->m_oConstConfig.m_uiLimit;
}
