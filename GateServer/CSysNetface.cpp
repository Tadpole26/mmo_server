	...
	uint32_t uiWaitNum, std::string strIp, uint32 uiPort, uint32 uiAreaNo)
{
	...
	oRecord.set_uimaxnum(GATE_LOGIC_INS->m_oConstConfig.m_uiLimit);
	oRecord.set_strversion(GATE_LOGIC_INS->GetResVersion());
...
{
	if (!GATE_LOGIC_INS->m_oUserMgr.GetLoadFinish())
		return;

	UpdateOnlinePlayerNum(GATE_LOGIC_INS->GetIndex(), (uint32)GATE_LOGIC_INS->m_oUserMgr.GetInGamePlayerSize(),
		(uint32)GATE_LOGIC_INS->m_oUserMgr.GetWaitCheckPlayerSize(), GATE_LOGIC_INS->m_oConstConfig.m_strGateIp,
		GATE_LOGIC_INS->m_oConstConfig.m_uiGatePort, GATE_LOGIC_INS->m_oConstConfig.m_uiGroupId);
}
...
void CSysNetface::on_connect()
{
	...
	oRegister.set_uiplatid(GATE_LOGIC_INS->m_oConstConfig.m_uiPlatId);
	oRegister.set_uiindex(GATE_LOGIC_INS->GetIndex());
	oRegister.set_uigroupid(GATE_LOGIC_INS->m_oConstConfig.m_uiGroupId);
...
{
	GATE_LOGIC_INS->SendWChatMsg("Gate", "Sys", "disconnect");
...
void CSysNetface::OnPlayerLoginRet(const tagMsgHead* pMsgHead)
{
	...
	CUser* pUser = GATE_LOGIC_INS->m_oUserMgr.GetCheckingPwdPlayer(llUserId);
	if (!pUser)
	{
	...
	if (ResultCode::Code_Common_Success != eRet)
	{
		...
		GATE_LOGIC_INS->m_pClientLIF->OnKickConnect(pUser, false);
	}
	else
	{
...
void CSysNetface::OnKickPlayer(const tagMsgHead* pMsgHead)
{
	...
	CUser* pUser = GATE_LOGIC_INS->m_oUserMgr.GetPlayer(llUid);
	if (pUser == nullptr)
	{
