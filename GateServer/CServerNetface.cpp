...
void CClientLogic::OnVerifyAccount(const tagMsgHead* pHeadMsg, const tagHostHd& hd)
{
	...
	if (GATE_LOGIC_INS->m_oUserMgr.GetWaitCheckPlayer(llUserId) != nullptr ||
		GATE_LOGIC_INS->m_oUserMgr.GetCheckingPlayer(llUserId) != nullptr ||
		GATE_LOGIC_INS->m_oUserMgr.GetCheckingPwdPlayer(llUserId) != nullptr)
	{
		//正在加载游戏的直接杀掉
		send_io_conn_msg(hd, cs_layer_kill, 0, 0);
	...
	bool bReKick = false;
	//已经登录的直接踢下线
	CUser* pUser = GATE_LOGIC_INS->m_oUserMgr.GetInGamePlayer(llUserId);
	if (pUser != nullptr)
	{
	...
	//加入排队队列
	pUser = GATE_LOGIC_INS->m_oUserMgr.AddNewWaitCheckPlayer(llUserId);
	...
	if (bReKick || pUser->m_uiReLogin)
	{
		...
		GATE_LOGIC_INS->m_oUserMgr.ReLoginUser(pUser);
	}
	else
	{
