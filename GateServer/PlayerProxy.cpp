...
void CPlayerProxy::CheckingSessionFromLoginSvr(CUser* pUser)
{
	...
	GATE_LOGIC_INS->m_pSysNetface->PlayerLogin(pUser->m_llUid, pUser->m_strSessionId);
}
...
void CPlayerProxy::LogicToGameServer(CUser* pUser, uint32_t uiGameId)
{
	...
	oLoginReq.set_uigateid(GATE_LOGIC_INS->m_oConstConfig.m_uiGroupId);
	...
	GATE_LOGIC_INS->m_pGameNetface->Send_Msg(&oLoginReq,
	...
	GATE_LOGIC_INS->m_oUserMgr.AddCheckingUser(pUser->m_llUid);
}

void CPlayerProxy::NotifyPlayerOnline(CUser* pUser)
{
	GATE_LOGIC_INS->m_pSysNetface->NotifyPlayerOnline(
		GATE_LOGIC_INS->GetIndex(), pUser->m_llUid);
}

void CPlayerProxy::NotifyPlayerOffline(CUser* pUser)
{
	GATE_LOGIC_INS->m_pSysNetface->NotifyPlayerOffline(
		GATE_LOGIC_INS->GetIndex(), pUser->m_llUid);
}
...
void CPlayerProxy::SendToGameSvr(CUser* pUser, const tagMsgHead* pNetMsgHead)
{
	...
	TransmitToServer(GATE_LOGIC_INS->m_pGameNetface, pUser, pNetMsgHead);
...
void CPlayerProxy::OnCreatePlayer(CUser* pUser, const tagMsgHead* pNetMsgHead)
{
	...
	GATE_LOGIC_INS->m_pGameNetface->Send_Msg(&oCreateReq,
...
void CPlayerProxy::TransimitToClient(const tagMsgHead* pMsg)
{
	...
	{
		GATE_LOGIC_INS->m_oUserMgr.SendToPlayer(oTransmit.lluid(), pNetMsgHead);
	}
	else
	{
		if (oTransmit.vecuids_size() > 0)
		{
			for (auto& value : oTransmit.vecuids())
				GATE_LOGIC_INS->m_oUserMgr.SendToPlayer(value, pNetMsgHead);
		}
		else
		{
			GATE_LOGIC_INS->m_oUserMgr.SendToAllPlayer(pNetMsgHead);
		}
	}
}
