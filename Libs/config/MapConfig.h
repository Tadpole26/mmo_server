...
namespace config
{
	...
	public:
		...
		const map_info_t* getMapConfig(const uint32 tid) const;
		size_t sizeMapCfg() const { return _allMapCfg.size(); }
		bool foreach(std::function<bool (map_info_t* pConfig)> func) const;
	private:
		bool AddMapInfo();

	private:
		std::map<uint32_t, map_info_t*> _allMapCfg;
	};
}
